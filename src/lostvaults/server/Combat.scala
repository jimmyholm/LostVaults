...
object Combat {
  def props(dungeon: ActorRef, roomIndex: Int, id: Int): Props = Props(new Combat(dungeon: ActorRef, roomIndex: Int, id:Int))
}


class Combat (_dungeon: ActorRef, _roomIndex: Int, id: Int) extends Actor with FSM[CombatState, CombatData] {
  ...
  when(Rest, stateTimeout = 500.milliseconds) {
    case Event(StateTimeout, data: RestData) => {
      ...
      if (nextTurnList isEmpty) {
        println("(Combat " + myID + ") COMBAT-Rest: New round, no player up for their turn.")
        stay using RestData(data.PlayerList, nextDuration, data.CombatsPerPlayer)
      } else {
        println("(Combat " + myID + ") COMBAT-Rest: New round, players " + nextTurnList + " up for their turn.")
        val nextPlayer = nextTurnList.head
        println("(Combat " + myID + ") COMBAT-Rest: It's " + nextPlayer + "'s turn to take an action.")
    ...
    case Event(AddPlayer(nameActorRef, name, speed, enemy), incomingData: RestData) => {
      println("(Combat " + myID + ") AddPlayer received for: " + name)
  ...
  when(WaitForAck) {
    case Event(DamageAck, data: ActionData) => {
      println("(Combat " + myID + ") COMBAT-WaitForAck-ActionAck: ActionAck Received")
      val nextTurnList = data.TurnList.tail
      if (nextTurnList isEmpty) {
        println("(Combat " + myID + ") COMBAT-WaitForAck: TurnList is empty, returning to rest state.")
        goto(Rest) using RestData(data.PlayerList, data.Duration, data.CombatsPerPlayer)
      } else {
        println("(Combat " + myID + ") COMBAT-WaitForAck: TurnList: " + data.TurnList + " nextTurnList: " + nextTurnList)
        val nextPlayer = nextTurnList.head
        println("(Combat " + myID + ") COMBAT-WaitForAck: It's " + nextPlayer + "'s turn to take an action.")
        combatSendGameMsg(nextPlayer, data.PlayerList, GameYourTurn)
    ...
    case Event(RemovePlayer(name), data: ActionData) => {
      println("(Combat " + myID + ") COMBAT-WaitForAck: Remove Player called for " + name + ".")
      ...
      println("(Combat " + myID + ") COMBAT-WaitForAck-RemovePlayer: Players still in combat: " + nextPlayerList)
      combatList = combatList filterNot (c => c._2 isEmpty)
      println("(Combat " + myID + ") COMBAT-WaitForAck-RemovePlayer: RemovePlayer combatList ActionData: " + combatList)
      ...
      if (nextTurnList isEmpty) {
        println("(Combat " + myID + ") COMBAT-WaitForAck-RemovePlayer: TurnList is empty, returning to rest state.")
        goto(Rest) using RestData(nextPlayerList, data.Duration, combatList)
      } else {
        println("(Combat " + myID + ") COMBAT-WaitForAck-RemovePlayer: TurnList: " + data.TurnList + " nextTurnList: " + nextTurnList)
  ...
  when(Action) {
    case Event(AttackPlayer(name, target, strength), data: ActionData) => {
      println("(Combat " + myID + ") COMBAT: AttackPlayer received")
      val turnPlayer = data.TurnList.head
      if (data.PlayerList.exists(c => c._1.compareToIgnoreCase(target) == 0)) {
        println("(Combat " + myID + ") COMBAT: Target exists")
        if (name == turnPlayer) {
          println("(Combat " + myID + ") COMBAT: name == turnplayer")
          println("(Combat " + myID + ") COMBAT-Action-AttackPlayer: Attack message received from the player whose turn it is.")
          combatSendGameMsg(target, data.PlayerList, GameDamage(name, strength))
          goto(WaitForAck)

        } else {
          println("(Combat " + myID + ") COMBAT-Action-AttackPlayer: Attack message from a player whose turn it is not.")
          stay
        }
      } else {
        println("(Combat " + myID + ") COMBAT-Action-AttackPlayer: Tried to attack a player that is not in combat")
    ...
    case Event(DrinkPotion(name), data: ActionData) => {
      ...
      val nextTurnList = data.TurnList.tail
      if (name == turnPlayer) {
        println("(Combat " + myID + ") COMBAT-Action-DrinkPotion: DrinkPotion message received from the player whose turn it is.")
        combatSendGameMsg(turnPlayer, data.PlayerList, GameDrinkPotion)
        if (nextTurnList isEmpty) {
          println("(Combat " + myID + ") COMBAT-Action-DrinkPotion: TurnList is empty, returning to rest state.")
          goto(Rest) using RestData(data.PlayerList, data.Duration, data.CombatsPerPlayer)
        } else {
          val nextPlayer = nextTurnList.head
          println("(Combat " + myID + ") COMBAT-Action-DrinkPotion: TurnList: " + data.TurnList + " nextTurnList: " + nextTurnList)
          println("(Combat " + myID + ") COMBAT-Action-DrinkPotion: It's " + nextPlayer + "'s turn to take an action.")
          combatSendGameMsg(nextPlayer, data.PlayerList, GameYourTurn)
          goto(Action) using ActionData(data.PlayerList, nextTurnList, data.Duration, data.CombatsPerPlayer)
        }
      } else {
        println("(Combat " + myID + ") COMBAT-Action-DrinkPotion: DrinkPotion message from a player whose turn it is not.")
        stay
      }
    }
    case Event(AddPlayer(nameActorRef, name, speed, enemy), data: ActionData) => {
  ...
  def addPlayer(nameActorRef: ActorRef, name: String, speed: Int, enemy: String, incomingData: CombatData) = {
    println("(Combat " + myID + ") COMBAT-addPlayerFunction: Adding player " + name + " with speed " + speed + ".")
    ...
    println("(Combat " + myID + ") COMBAT-addPlayerFunction: Addplayer combatList: " + combatList)
    if (data.PlayerList.exists(x => x._1.compareToIgnoreCase(name) == 0)) {
      println("(Combat " + myID + ") COMBAT-addPlayerFunction: no new player. PlayerList: " + data.PlayerList + " TurnList: " + data.TurnList + " combatList: " + combatList)
      ...
      println("(Combat " + myID + ") COMBAT-addPlayerFunction: New player was added. PlayerList: " + nextPlayerList + " TurnList: " + data.TurnList + " combatList: " + combatList)
